...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Bestcord")
	var updateFlag = flag.Bool("repair", false, "Repair Bestcord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Bestcord")
	...
	if *versionFlag {
		fmt.Println("Bestcord Installer Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Bestcord Installer' option to update, or run with --update-self")
			}
		}()
		...
		case "Quit":
			return
		case "Update Bestcord Installer":
			if err := UpdateSelf(); err != nil {
				Log.Error("Failed to update self:", err)
				exitFailure()
			}
	...
	if install {
		...
		Log.Info("Downloading latest Bestcord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Bestcord will likely not work.")
}
